...
class DeqTransactionTests(TestCase):
    ...
    def test_create_to_dai_transaction(self):
        ...
        # it fails if eth_address is missing
        response = self._api_create_transaction(200, TRANSACTION_CATEGORY_CHOICES.TO_DAI, {}, user)
        self.assertEqual(400, response.status_code)
        self.assertEqual('A valid eth address was not provided', response.json()['non_field_errors'][0])

        # it fails if eth_address is not valid
        response = self._api_create_transaction(200, TRANSACTION_CATEGORY_CHOICES.TO_DAI, {'eth_address': '1234'}, user)
        self.assertEqual(400, response.status_code)
        self.assertEqual('A valid eth address was not provided', response.json()['non_field_errors'][0])

        # it fails if user does not have enough DEQ
        response = self._api_create_transaction(200, TRANSACTION_CATEGORY_CHOICES.TO_DAI, {'ethereum_address': TEST_ETH_ADDRESS}, user)
        self.assertEqual(400, response.status_code)
        self.assertEqual('Not enough DEQ to make transaction', response.json()['non_field_errors'][0])
    ...
    def test_create_to_dai_transaction(self):
        ...
        response = self._api_create_transaction(200, TRANSACTION_CATEGORY_CHOICES.TO_USD, {}, user)
        self.assertEqual(400, response.status_code)
        self.assertEqual('A valid email was not provided', response.json()['non_field_errors'][0])

        # it fails if email_address is not valid
        response = self._api_create_transaction(200, TRANSACTION_CATEGORY_CHOICES.TO_USD, {'paypal_email': '1234'}, user)
        self.assertEqual(400, response.status_code)
        self.assertEqual('A valid email was not provided', response.json()['non_field_errors'][0])

        # it fails if user does not have enough DEQ
        response = self._api_create_transaction(200, TRANSACTION_CATEGORY_CHOICES.TO_USD, {'paypal_email': 'paypal@mafia.com'}, user)
        self.assertEqual(400, response.status_code)
        self.assertEqual('Not enough DEQ to make transaction', response.json()['non_field_errors'][0])
